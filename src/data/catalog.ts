...
// Same `placeName|category` always yields the same sequence, so deal prices are
// reproducible across renders, reloads and client/server (no Math.random()).
...
const DEEP_LINKS: Record<string, (name: string) => string> = {
  meituan:  (n) => `https://s.meituan.com/${encodeURIComponent(n)}`,
  douyin:   (n) => `https://www.douyin.com/search/${encodeURIComponent(n)}`,
  dianping: (n) => `https://www.dianping.com/search/keyword/${encodeURIComponent(n)}`,
  taobao:   (n) => `https://s.taobao.com/search?q=${encodeURIComponent(n)}`,
  xianyu:   (n) => `https://s.ershou.taobao.com/search.htm?q=${encodeURIComponent(n)}`,
};

export function generateDeals(placeName: string, category: Category): DealsResult {
  ...
  const r = (lo: number, hi: number) => Math.round(base * lo + rand() * base * (hi - lo));
  ...
  const deals: Deal[] = [
    ...
    return {
      ...d,
      isBest: d.price === minP,
      platformName: p?.name ?? d.platform,
      platformColor: p?.color ?? "#888888",
      discount: Math.round(((base - d.price) / base) * 100),
      deepLink: DEEP_LINKS[d.platform]?.(placeName) ?? "",
    };
  });
